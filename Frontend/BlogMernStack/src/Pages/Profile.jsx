...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    try {
      const res = await axios.get(URL + "/api/users/" + userr._id);
  ...
  const handleUserDelete = async () => {
    try {
      const res = await axios.delete(URL + "/api/users/" + userr._id, {
        withCredentials: true,
      });
      setUserr(null);
      navigate("/");
  ...
  return (
    <div>
      <Nav />
      <div className="min-h-[80vh] px-8 md:px-[100px] mt-8 flex md:flex-row flex-col-reverse md:items-start items-start">
